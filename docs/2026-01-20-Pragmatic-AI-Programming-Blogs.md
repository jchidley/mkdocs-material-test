...
- **Lilian Weng**: VP Research at OpenAI. Comprehensive, well-cited deep dives.
- **Jay Alammar**: "The Illustrated Transformer" - referenced in MIT/Cornell courses.
...
- Critical of hype but not dismissive
- Strong "show me the code" energy

---

## Checking for Updates

To check when these blogs were last updated, use the prompt below with an AI assistant that has web access, or run the check script.
...
### Check Script (using Brave Search skill)

Save and run `check-blogs.sh` from the same directory:
...
for blog in "${blogs[@]}"; do
    result=$("$SKILL_DIR/search.js" "site:${blog%% *} ${blog#* }" -n 1 2>/dev/null | grep -E "^(Title|Age):" | head -2)
    title=$(echo "$result" | grep "Title:" | sed 's/Title: //')
    age=$(echo "$result" | grep "Age:" | sed 's/Age: //')
    domain=$(echo "$blog" | cut -d' ' -f1)
    echo "| $domain | ${title:-N/A} | ${age:-N/A} |"
done
```

Make executable: `chmod +x check-blogs.sh`

---

*Last curated: 2026-01-20*
