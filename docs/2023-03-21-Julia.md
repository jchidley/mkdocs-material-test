...
Everyting in julia is based on packages. Even the language server (at least in neovim) will not work without being run from a package. 
...
For a whole stack of reasons, it's best to use Julia's inbuilt Miniconda enviroment.The following code will need to be added to the top of any script, otherwise PyCall will use Julia's conda but the rest of the script will be using whatever the system's python is.
...
* [Smallcheck documentaiton](https://hackage.haskell.org/package/smallcheck-1.2.1/docs/Test-SmallCheck.html)
...
```julia
using Pkg
Pkg.status() # what Pkg enviroment am I using, what packages have been installed?
Pkg.activate(".") # activate enviroment in current directory, or set it up 
Pkg.add("PythonCall")
